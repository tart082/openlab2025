...
rocket_image = pygame.image.load(
    "images/space_saturnV_rocket_apollo11.png")  # (261*500)
rocket_image = pygame.transform.scale(rocket_image, (26, 50))  # 画像を縮小
alien_bullet_image = pygame.image.load(
    "images/sports_balance_ball.png")  # (400*400)
alien_bullet_image = pygame.transform.scale(alien_bullet_image, (10, 10))
silver_bullet_image = pygame.image.load(
    "images/gin_dangan_silver_bullet.png")  # (262*400)
...
pointing_up_image = pygame.image.load(
    "images/pose_hitosashiyubi.png")  # (290*400)
...
def main(t=60, full_screen=False):
    ...
    if full_screen:
        screen = pygame.display.set_mode(
            SCREEN_SIZE, pygame.FULLSCREEN)  # フルスクリーン表示
    else:
    ...
    while running and cap.isOpened():
        ...
        if current_gesture == "Pointing Up" and player.remaining_shot_cool_time == 0 and not game_over and not game_clear and game_started:
            ...
            if player.barrier_stock > 0 and len(barriers) == 0:
                ...
                barrier = Barrier(player.rect.centerx,
                                  player.rect.top)  # バリアが自機の位置に出る
                ...
                for alien in aliens:
                    if random.random() <= probability:
                        alien_bullet = AlienBullet(
                            alien.rect.centerx, alien.rect.bottom, speed_mag)  # 敵から弾が出る
                        alien_bullet.angle(
                            player.rect.x, player.rect.y)  # 角度を変える
        ...
        if not game_over and not game_clear and game_started:
            ...
            pygame.sprite.groupcollide(
                barriers, alien_bullets, False, True)  # バリアにあたった弾は消える
            ...
            for alien_bullet in alien_bullets:
                player_hits = pygame.sprite.collide_circle(
                    player, alien_bullet)  # 円で判定
                if player_hits:
                    game_over = True
                    select_timer = select_timer_max
            # 生存ボーナス得点増加
        ...
        frame_surface = pygame.surfarray.make_surface(
            np.flip(frame, axis=-1))  # axis=1で左右反転（カメラに合わせる）

        # サーフェスをscreenにblit（描画）
        screen.blit(pygame.transform.rotate(
            frame_surface, -90), (0, 0))  # 必要なら回転も
        ...
        if select_timer < select_timer_max and (not game_started or game_over or game_clear):
            x, y = player.finger
            pygame.draw.arc(screen, RED, (x - 25, y - 25, 50, 50), math.pi / 2, 2 * math.pi * (
                (select_timer_max - select_timer) / (select_timer_max)) + math.pi / 2, 5)
        ...
        if game_started and not game_over and not game_clear:
            ...
            for i in range(player.barrier_stock + 1):
                if i == 0:
                    pygame.draw.arc(screen, GREEN, (x - r, y - r, r * 2, r * 2), math.pi / 2, 2 * math.pi * (
                        (player.barrier_cool_time - player.barrier_remaining_cool_time) / (player.barrier_cool_time)) + math.pi / 2, w)
                else:
                    pygame.draw.arc(
                        screen, GREEN, (x - r, y - r, r * 2, r * 2), 0, 2 * math.pi, w)
                r -= 8
            ...
            pygame.draw.arc(screen, GREEN, (x - r, y - r, r * 2, r * 2), math.pi / 2, 2 * math.pi * (
                (player.shot_cool_time - player.remaining_shot_cool_time) / (player.shot_cool_time)) + math.pi / 2, w)
